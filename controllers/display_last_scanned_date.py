...
async def display_last_scanned_date(records):
    ...
    result_df = df[['IP Address', 'Port', 'Plugin Modification Date']].copy()
    result_df = result_df.rename(columns={'Plugin Modification Date': 'Last Scanned'})

    # Sort by Last Scanned (latest on top)
    result_df = result_df.sort_values(by='Last Scanned', ascending=False)
    ...
    return {
        "intent": "display_last_scanned_date",
        "type": "table",
        "plot_type": "table",
        "title": "Last Scanned Date per Asset",
        "columns": ['IP Address', 'Port', 'Last Scanned'],
        "data": data,
        "graph": table_image
    }
