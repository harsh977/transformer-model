...
async def display_highest_risk_assets(records: List[Dict], top_n: int = 10) -> Dict:
    ...
    bar_data = top_assets_vulns.groupby(['asset_id', 'Severity']).size().unstack(fill_value=0)[severity_order]
    
    # Sort by risk score
    bar_data = bar_data.loc[top_assets['asset_id']]
    
    # Create stacked bar chart
    plt.figure(figsize=(12, 8))
    colors = {'Critical': '#d62728', 'High': '#ff7f0e', 'Medium': '#f7e11a', 'Low': '#2ca02c'}
    bar_data.plot(kind='barh', stacked=True, color=[colors[sev] for sev in severity_order])
    
    plt.title(f'Top {top_n} Highest Risk Assets\n(Severity Breakdown)', pad=20)
    ...
    plt.xticks(ticks=range(0, int(bar_data.sum(axis=1).max()) + 2, 2))
    ...
    buffer.seek(0)
    bar_chart_image = base64.b64encode(buffer.read()).decode('utf-8')
    buffer.close()
    plt.close()
     
    return {
        "type": "image",
        "graph": bar_chart_image
    }